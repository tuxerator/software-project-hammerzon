...
export default class ProductTestData {
  ...
  async insertIfNotExistend(): Promise<void> {
    ...
    {
      const newImg = await db.Image.insertMany([{
        type:'image/png',
    ...
    const categoryIds = await db.Category.insertMany(categoryLists);

    await db.Product.deleteMany({});

    const users = await db.Image.find({});
    console.log(users.map(data => data._id));


    const vals: IProduct[] = await db.Product.find({});
    if (!vals || vals.length <= 0) {
      ...
      console.log(newList);
      await db.Product.insertMany(newList);
      const current: IProduct[] = await db.Product.find({}).populate('user', '-password').exec();
      console.log(current);
    } else {
