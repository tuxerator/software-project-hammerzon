...
export class LandingpageService {
  ...
  getProductList(start:number = 0, limit:number = 10,search:string=''): Observable<ListInfo<Product>>  {
    console.log('Hallo ich komm dahin');
    if(search === '')
    {
      return this.http.get<ListInfo<Product>>(`api/productlist?start=${start}&limit=${limit}`);
    }else{
      return this.http.get<ListInfo<Product>>(`api/productlist?start=${start}&limit=${limit}&search=${search}`);
    }
  }
}
