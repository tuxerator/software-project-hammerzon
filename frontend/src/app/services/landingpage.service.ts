...
export class LandingpageService {
  ...
  getProductList(start:number = 0, limit:number = 10,search:string=''): Observable<ListInfo<ProductInfo>>  {
    console.log('Hallo ich komm dahin');
    if(search === '')
    {
      return this.http.get<ListInfo<ProductInfo>>(`api/productlist?start=${start}&limit=${limit}`);
    }else{
      return this.http.get<ListInfo<ProductInfo>>(`api/productlist?start=${start}&limit=${limit}&search=${search}`);
    }
  }
}
